...
const ProductCarousel: React.FC<CarouselProps> = ({
  ...
  const [canScrollLeftPersonalized, setCanScrollLeftPersonalized] = useState(false);
  const [canScrollRightPersonalized, setCanScrollRightPersonalized] = useState(true);
 ...
 // Función para desplazar a la izquierda
  ...
  useEffect(() => {
    ...
    const handlePersonalizedScroll = () => {
      if (personalizedEl) {
        checkScrollability(personalizedEl, setCanScrollLeftPersonalized, setCanScrollRightPersonalized);
      }
    };

    const handleTrendingScroll = () => {
      if (trendingEl) {
        checkScrollability(trendingEl, setCanScrollLeftTrending, setCanScrollRightTrending);
      }
    };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="relative">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-2">
            <button
              onClick={() => scrollLeft(personalizedCarouselRef as React.RefObject<HTMLDivElement>)}
              disabled={!canScrollLeftPersonalized}
              className={`p-1 rounded-full border ${
                canScrollLeftPersonalized ? 'text-gray-700 border-gray-300 hover:bg-gray-100' : 'text-gray-400 border-gray-200 cursor-not-allowed'
              }`}
            >
              <ChevronLeft size={20} />
            </button>
            <button
              onClick={() => scrollRight(personalizedCarouselRef as React.RefObject<HTMLDivElement>)}
              disabled={!canScrollRightPersonalized}
              className={`p-1 rounded-full border ${
                canScrollRightPersonalized ? 'text-gray-700 border-gray-300 hover:bg-gray-100' : 'text-gray-400 border-gray-200 cursor-not-allowed'
              }`}
            >
      ...
      <div className="relative">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold text-gray-800">Ofertas y tendencias</h2>
          <div className="flex gap-2">
            <button
              onClick={() => scrollLeft(trendingCarouselRef as React.RefObject<HTMLDivElement>)}
              disabled={!canScrollLeftTrending}
              className={`p-1 rounded-full border ${
                canScrollLeftTrending ? 'text-gray-700 border-gray-300 hover:bg-gray-100' : 'text-gray-400 border-gray-200 cursor-not-allowed'
              }`}
            >
              <ChevronLeft size={20} />
            </button>
            <button
              onClick={() => scrollRight(trendingCarouselRef as React.RefObject<HTMLDivElement>)}
              disabled={!canScrollRightTrending}
              className={`p-1 rounded-full border ${
                canScrollRightTrending ? 'text-gray-700 border-gray-300 hover:bg-gray-100' : 'text-gray-400 border-gray-200 cursor-not-allowed'
              }`}
            >
              <ChevronRight size={20} />
            </button>
