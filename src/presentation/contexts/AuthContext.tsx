...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const fetchRoleInfo = useCallback(async () => {
    ...
    // Evitamos consultas repetidas si ya estamos cargando
    if (isLoadingRole) {
      return;
    }
    ...
    try {
      ...
      if (roleData && roleData.success) {
        ...
        // Solo actualizar si hay cambios para evitar re-renderizaciones
        if (JSON.stringify(newRoleInfo) !== JSON.stringify(roleInfo)) {
  ...
  }, [isAuthenticated, user, isLoadingRole]);
  ...
  // Efecto separado para cargar los roles solo cuando cambia el estado de autenticación
  useEffect(() => {
    // Solo consultar roles si el usuario está autenticado y ya se inicializó
    if (isAuthenticated && initialized) {
      fetchRoleInfo();
    }
  }, [isAuthenticated, initialized, fetchRoleInfo]);
